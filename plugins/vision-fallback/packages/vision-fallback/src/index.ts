...
import type { Agent, ImageInputFallback, ModelSelection, PreStepDecision } from '@deepseek-ai/dsh-agent'
...
declare module '@deepseek-ai/dsh-llm' {
  interface MessageSourceMap {
    ...
    'vision-fallback-display': { kind: 'user'; visionFallback: 'display' }
...
function selectedModel(agent: Agent): ModelSelection {
...
function withoutImages(
  ...
  imageCount: number,
  displaySeqs: ReadonlyMap<string, number>,
): UserMessage[] {
...
function appendDisplayCopies(agent: Agent, messages: readonly UserMessage[]): ReadonlyMap<string, number> {
  const displaySeqs = new Map<string, number>()
  for (const message of messages) {
    if (message.source.kind !== 'user' || !message.content.some(block => block.type === 'image')) continue
    const display = createUserMessage({
      content: message.content,
      source: { kind: 'user', visionFallback: 'display' as const },
    })
...
export class VisionFallback extends Service implements ImageInputFallback {
  static inject = ['llm']
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.source = () => entry
    installSettingsSection(ctx, VISION_FALLBACK_SETTINGS_NAMESPACE, Config as z<ResolvedConfig>, entry, {
    ...
    ctx.on('agent/pre-step', async ({ agent, signal }, next): Promise<PreStepDecision> => {
      ...
      if (mainInfo.inputModalities === undefined || mainInfo.inputModalities.includes('image')) return decision
  ...
  async canHandle(selection: ModelSelection): Promise<boolean> {
    const config = this.source()
