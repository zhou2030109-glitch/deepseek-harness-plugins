...
const install = (ctx, fail) => {
    ctx.on('internal/dispatch', (_mode, eventName, args) => {
        if (eventName !== 'session/event')
            return;
        const event = args[1];
        if (event.type === 'vision-fallback/display')
            return validateDisplay(event, fail);
        if (event.type !== 'user/message')
            return;
        const marker = event.data.source.visionFallback;
        if (marker === 'model-input')
            return validateModelInput(event, fail);
